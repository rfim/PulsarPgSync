...
class PostgresSync(Function):
    ...
    def process(self, input, context):
        """
        Process input data originating from Debezium and sync it to PostgreSQL.
        
        The function assumes the input data is structured according to Debezium's output for PostgreSQL changes. 
        Specifically, it looks for the 'after' and 'source' fields in the input to process the data.
        """
        
        error_messages = []

        context.get_logger().info(f"Received Debezium input: {input}")
        ...
        try:
            ...
            err_msg = f"Error during data insertion into PostgreSQL. Data from Debezium: {input}. Error: {e}"
            context.get_logger().error(err_msg)
            error_messages.append(err_msg)
